...
import { renderResearchBody } from '../../lib/research-posts';
...
export default function ResearchPostView({ slug }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setState({ kind: 'ready', post: data as ResearchPost });
    })();
    return () => {
      cancelled = true;
    };
  ...
  if (state.kind === 'not-found' || state.kind === 'error') {
    const heading = state.kind === 'not-found' ? 'Entry not found.' : 'Entry temporarily unavailable.';
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 md:px-6 py-20 md:py-28">
        ...
        <p data-i18n={i18nAttr('The entry you are looking for may have been moved or unpublished. Browse all our latest R&D updates below.')} className="mt-4 text-base text-ink/75 leading-relaxed">
          The entry you are looking for may have been moved or unpublished. Browse all our latest R&D updates below.
        </p>
        <a
          href="/innovation/research-development/"
        ...
        >
          <span data-i18n={i18nAttr('Back to Research & Development')}>Back to Research & Development</span>
  ...
  return (
    <article>
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-6 py-12 md:py-20">
        <p data-i18n={i18nAttr('Innovation · R&D')} className="text-[11px] uppercase tracking-[0.4em] text-brand-500 font-semibold">Innovation · R&D</p>
        <h1
        ...
        <div
          ...
          // Sanitized via DOMPurify in renderResearchBody.
          dangerouslySetInnerHTML={{ __html: renderResearchBody(post.body) }}
        />
