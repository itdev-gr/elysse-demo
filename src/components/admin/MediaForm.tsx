...
import { slugify, uploadMediaImage } from '../../lib/media';
import type { Media, MediaDraft } from '../../types/media';
import { triggerPublish } from '../../lib/publish';

const MAX_BYTES = 4 * 1024 * 1024;
const ALLOWED_MIME = ['image/jpeg', 'image/png', 'image/webp'];

type Props = { initial?: Media; onSaved: () => void; onCancel: () => void };

function emptyDraft(): MediaDraft {
  return { slug: '', title: '', excerpt: '', body: '', video_url: '', poster_image: null, image_alt: null, is_published: true };
}
function toDraft(a: Media): MediaDraft {
  const { id: _id, created_at: _c, updated_at: _u, ...rest } = a; return rest;
}

export default function MediaForm({ initial, onSaved, onCancel }: Props) {
  const [draft, setDraft] = useState<MediaDraft>(() => (initial ? toDraft(initial) : emptyDraft()));
  const [pendingFile, setPendingFile] = useState<File | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(initial?.poster_image ?? null);
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const update = <K extends keyof MediaDraft>(k: K, v: MediaDraft[K]) => setDraft((d) => ({ ...d, [k]: v }));
  ...
  const removeImage = () => { setPendingFile(null); setPreviewUrl(null); update('poster_image', null); if (fileInputRef.current) fileInputRef.current.value = ''; };
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      let row: Media;
      if (initial) {
        const { data, error: err } = await supabase.from('media').update(payload).eq('id', initial.id).select().single();
        if (err) throw err; row = data as Media;
      } else {
        const { data, error: err } = await supabase.from('media').insert(payload).select().single();
        if (err) throw err; row = data as Media;
      }
      if (pendingFile) {
        const { url } = await uploadMediaImage(pendingFile, row.id);
        const { error: err } = await supabase.from('media').update({ poster_image: url }).eq('id', row.id);
        if (err) throw err;
      }
  ...
  return (
    <form onSubmit={onSubmit} className="bg-surface border-l-4 border-brand-500 p-6 md:p-8 space-y-5">
      <header className="flex items-center justify-between">
        <h2 className="font-display font-heavy text-xl text-ink">{initial ? 'Edit video' : 'New video'}</h2>
      ...
      <Field label="Poster image" hint="JPEG, PNG, or WebP. Max 4 MB. Optional.">
      ...
      <div className="flex items-center gap-3 pt-2">
        <button type="submit" disabled={submitting}
          ...
          {submitting ? 'Saving…' : initial ? 'Save changes' : 'Create video'}
        </button>
      </div>
    </form>
  );
