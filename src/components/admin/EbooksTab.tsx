...
export default function EbooksTab() {
  ...
  const togglePublish = async (row: Ebook) => {
    const { error: err } = await supabase.from('ebooks')
      .update({ is_published: !row.is_published }).eq('id', row.id);
    if (err) return setError(err.message);
    await load(); triggerPublish();
  };
  const remove = async (row: Ebook) => {
    if (!confirm(`Delete "${row.title}"? This cannot be undone.`)) return;
    const { error: err } = await supabase.from('ebooks').delete().eq('id', row.id);
    if (err) return setError(err.message);
    await load(); triggerPublish();
  };
