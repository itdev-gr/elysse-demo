...
export default function NewsForm({ initial, onSaved, onCancel }: Props) {
  const [draft, setDraft] = useState<NewsDraft>(() => (initial ? toDraft(initial) : emptyDraft()));
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (pendingFile) {
        const { url } = await uploadNewsCover(pendingFile, article.id);
  ...
  return (
    <form onSubmit={onSubmit} className="bg-surface border-l-4 border-brand-500 p-6 md:p-8 space-y-5">
      ...
      <Field label="Slug" required hint="Lowercase letters, digits, hyphens. Used in the article URL.">
        <input
          type="text"
          required
          pattern="[a-z0-9-]+"
      ...
      <Field label="Excerpt" required hint={`${excerptCount}/300 characters — shown on the news card.`}>
        <textarea
          required
          rows={2}
          maxLength={300}
