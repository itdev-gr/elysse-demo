...
import { slugify, calcReadingMinutes, uploadResearchCover } from '../../lib/research-posts';
import type { ResearchPost, ResearchPostDraft } from '../../types/research-post';
...
type Props = {
  initial?: ResearchPost;
  onSaved: () => void;
  onCancel: () => void;
};

function emptyDraft(): ResearchPostDraft {
  return {
    slug: '',
    title: '',
    excerpt: '',
...
function toDraft(a: ResearchPost): ResearchPostDraft {
  // Strip server-managed columns; the form only writes the editable draft.
  const { id: _id, created_at: _ca, updated_at: _ua, ...rest } = a;
  return rest;
}

export default function ResearchPostForm({ initial, onSaved, onCancel }: Props) {
  const [draft, setDraft] = useState<ResearchPostDraft>(() => (initial ? toDraft(initial) : emptyDraft()));
  ...
  const update = <K extends keyof ResearchPostDraft>(key: K, value: ResearchPostDraft[K]) =>
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload: ResearchPostDraft = {
        ...draft,
        slug,
      ...
      if (pendingFile) {
        const { url } = await uploadResearchCover(pendingFile, post.id);
  ...
  return (
    <form onSubmit={onSubmit} className="bg-surface border-l-4 border-brand-500 p-6 md:p-8 space-y-5">
      ...
      <Field label="Slug" required hint="Lowercase letters, digits, hyphens. Used in the entry URL.">
        <input
          type="text"
          required
          pattern="[a-z0-9-]+"
      ...
      <Field label="Excerpt" required hint={`${excerptCount}/300 characters — shown on the R&D card.`}>
        <textarea
          required
          rows={2}
          maxLength={300}
      ...
      <Field label="Publish date" hint="Defaults to now when first published.">
        <input
          ...
          onChange={(e) =>
            update('published_at', e.currentTarget.value ? new Date(e.currentTarget.value).toISOString() : null)
          }
          className={inputClass}
        />
      </Field>
