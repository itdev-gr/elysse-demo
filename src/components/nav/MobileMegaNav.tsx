...
export default function MobileMegaNav({ groups }: Props) {
  ...
  // always English); the stored language is applied right after mount. See the
  // matching comment in MegaNav.tsx.
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={tFor(lang, 'Primary navigation')}
          ...
          >
            ...
            >
              <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-5 border-b border-ink/10 bg-surface">
                <span className="text-sm uppercase tracking-widest font-medium">{tFor(lang, 'Menu')}</span>
                <button
                  type="button"
                  onClick={closeDrawer}
                  aria-label={tFor(lang, 'Close menu')}
                          ...
                          {hasItems && isExpanded && (
                            ...
                            >
                              <ul className="px-4 pb-4 space-y-3">
                                ...
                                {group.items.map((it) => (
                                  <MobileItem key={it.href} item={it} lang={lang} />
                                ))}
                              </ul>
                            </motion.div>
                          )}
...
function MobileItem({ item, lang }: { item: NavItem; lang: string }) {
