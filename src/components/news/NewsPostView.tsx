...
type State =
  ...
  | { kind: 'ready'; article: NewsArticle };
...
export default function NewsPostView({ slug }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setState({ kind: 'ready', article: data as NewsArticle });
    })();
    return () => {
      cancelled = true;
    };
  ...
  const author = article.author?.trim() || 'Elysée Group';
  const date = formatDate(article.published_at);
  const minutes = article.reading_minutes ?? 1;
    ...
    <article>
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-6 py-12 md:py-20">
        ...
        <div
          ...
          // Sanitized via DOMPurify in renderNewsBody.
          dangerouslySetInnerHTML={{ __html: renderNewsBody(article.body) }}
        />
