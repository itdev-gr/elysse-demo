...
function init(): () => void {
  ...
  const headerOffset = (): number => {
    // Header is fixed at 80px (5rem) on mobile, 96px (6rem) on md+
  ...
  const onWheel = (e: WheelEvent) => {
    ...
    // Coalesce rapid bursts of wheel events from trackpad inertia
    ...
    // it's the tail of inertia — swallow it.
    const direction: 1 | -1 = e.deltaY > 0 ? 1 : -1;
    if (sinceLast < 80 && direction === lastDirection) {
      e.preventDefault();
      return;
    }
