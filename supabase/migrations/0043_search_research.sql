-- 0043_search_research.sql — extend the site-wide search RPC to cover the new
-- public.research_posts table (R&D listing at /innovation/research-development/).
-- Full CREATE OR REPLACE of the CURRENT live public.search_site (which since
-- the family-owned-images work resolves the product image from
-- product_family_images, not the dropped products.image_url), with one added
-- UNION branch ('research') mirroring the posts/news branches.
